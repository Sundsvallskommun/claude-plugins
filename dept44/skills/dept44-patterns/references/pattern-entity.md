...
- **Every entity needs the full member set, no exceptions**: `create()`, a getter AND a setter AND a `with*()` for every field, plus manual `equals()`, `hashCode()`, `toString()` covering **all** fields — except LOB/binary fields (see next bullet). Don't ship a "fluent-only" entity (with* but no set*) — it's incomplete and **breaks `testBean`** (BeanMatchers' `hasValidGettersAndSetters`/`hasValidBeanEquals`/`hasValidBeanToString` mutate via setters, so a missing setter fails with `AccessorMissing missing setter for property …`).
- **LOB/binary fields (`Blob`, `byte[]`) are excluded from `equals()`, `hashCode()` and `toString()`.** `Objects.equals` on a `java.sql.Blob` is reference equality (meaningless), a content compare would read the whole longblob, and `toString` would print a lazy proxy or dump bytes. Compare on `id` (plus the small metadata fields) and print only the id, then mirror the exclusion in `testBean` with `hasValidBeanEqualsExcluding("file")`, `hasValidBeanHashCodeExcluding("file")` and `hasValidBeanToStringExcluding("file")` (BeanMatchers) — the plain `hasValidBeanEquals()` would fail because mutating the excluded property no longer changes equality.
